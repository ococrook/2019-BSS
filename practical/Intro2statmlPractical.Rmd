...
author:
- name: oliver M. Crook
  affiliation: MRC Biostatics Unity, Cambridge, UK
abstract: >
...
The syntax to include interaction terms is the following `lstat*age`. The `*` automatrically
include the terms `lstat` and `age` by default.
...
21) What can use deduce from the output about these predictors?
...
The `contrasts` function extrats the coding that was used for the factors.
...
such as empirical Bayes to improve robustness.
